   ...
   contract Eleicao {
       ...
       function votar(bytes32 eleitorId, bytes32 opcaoVotoId, string memory hashAnterior, string memory numeroBloco) public {
           require(!jaVotou[eleitorId], "Eleitor ja votou.");
           require(bytes(hashAnterior).length > 0, "Hash anterior e obrigatorio.");
           require(bytes(numeroBloco).length > 0, "Numero do bloco e obrigatorio.");
