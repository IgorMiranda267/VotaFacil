...
namespace VotaFacil.Infrastructure.Service
{
    public class EthereumService : IEthereumService
    {
        ...
        public async Task<List<VotoRegistradoEventDTO>> GetVotosRegistradosAsync(string transactionHash)
        {
            ...
            var eventHandler = _web3.Eth.GetEvent<VotoRegistradoEventDTO>();

            var logs = eventHandler.DecodeAllEventsForEvent(transactionReceipt.Logs);
            return logs.Select(log => log.Event).ToList();
        }
        #endregion ELEIÇÂO
